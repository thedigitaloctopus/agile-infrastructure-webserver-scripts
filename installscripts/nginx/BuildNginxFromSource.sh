...
./configure --prefix=/usr/local/pcre --with-openssl=/opt/openssl-${openssl_latest_version}
/usr/bin/make
/usr/bin/make install
cd ..
...
then
    ...
    /bin/sh build.sh
    ./configure --with-pcre=../pcre-${pcre_latest_version} --with-maxmind=no --with-openssl=/opt/openssl-${openssl_latest_version} 
    /usr/bin/make
    /usr/bin/make install
    cd ..
