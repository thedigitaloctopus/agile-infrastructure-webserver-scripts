...
then
    if ( [ -f ${HOME}/config/credentials/GATEWAY_GUARDIAN_UPDATED ] || [ ! -f /var/www/html/moodle/admin/.htpasswd ] )
    then
        /usr/bin/s3cmd get s3://gatewayguardian-${BUILD_IDENTIFIER}/htpasswd
        /bin/mv htpasswd /var/www/html/moodle/admin/.htpasswd
        /bin/chown www-data.www-data /var/www/html/moodle/admin/.htpasswd
        /bin/sleep 40
        /bin/rm ${HOME}/config/credentials/GATEWAY_GUARDIAN_UPDATED
    fi
fi
