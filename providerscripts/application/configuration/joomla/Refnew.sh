...
/bin/echo "${credentials}" > /tmp/credentials.$$

#credentials="`/bin/cat /tmp/credentials.$$ | /bin/sed 's/\./\\./g' | /bin/sed 's/\[/\\[/g' | /bin/sed 's/\*/\\*/g' | /bin/sed 's;/;\\/;g' | /bin/sed 's/\$/\\$/g' | /bin/sed 's/(/\\(/g' | /bin/sed 's/)/\\)/g'`"

/bin/cat /tmp/credentials.$$ | /bin/sed 's/\./\\./g' | /bin/sed 's/\[/\\[/g' | /bin/sed 's/\*/\\*/g' | /bin/sed 's;/;\\/;g' | /bin/sed 's/\$/\\$/g' | /bin/sed 's/(/\\(/g' | /bin/sed 's/)/\\)/g' > /tmp/credentialsprocessed.$$
...
#   credentials="`/bin/echo ${credentials} | /bin/sed "s/${username} /${username}\:gap1\:/g"`"
   /bin/cat /tmp/credentialsprocessed.$$ | /bin/sed "s/${username} /${username}\:gap1\:/g" > /tmp/credentialsdelimeted.$$
done
echo "22222222222"
for password in ${passwords}
do
#   credentials="`/bin/echo ${credentials} | /bin/sed "s/${password} /${password}\|gap2|/g"`"
   /bin/cat /tmp/credentialsprocessed.$$ | /bin/sed "s/${password} /${password}\:gap1\:/g" > /tmp/credentialsdelimeted.$$
done
