...
then
    ...
    then 
        ...
        then
            ...
            command="${SUDO} /bin/sed -i 's/\$this->toggleTables(false);/\/\/\$this->toggleTables(true);/g' /var/www/html/administrator/components/com_finder/src/Indexer/Indexer.php" && eval ${command} 
        fi
    else 
