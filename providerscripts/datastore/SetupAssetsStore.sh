...
if ( [ "`${HOME}/providerscripts/utilities/CheckConfigValue.sh PERSISTASSETSTOCLOUD:1`" != "1" ] && ( [ "`${HOME}/providerscripts/utilities/CheckConfigValue.sh PERSISTASSETSTOCLOUD:0`" != "1" ] || [ "`${HOME}/providerscripts/utilities/CheckConfigValue.sh PERSISTASSETSTOCLOUD:2`" != "1" ] ) )
then
    exit
fi
...
do
    ...
    then
        ...
        /bin/echo "${0} Shutdown initiated due to unresponsive S3FS mount ${directory_to_mount}" >> ${HOME}/logs/MOUNT_RECORD.log
