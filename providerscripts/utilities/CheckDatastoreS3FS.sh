...
while ( [ "${count}" -lt "5" ] )
do
    if ( [ -f ${HOME}/runtime/PERFORMING_S3FS_DATASTORE_CHECK ] && [ ! -f ${HOME}/runtime/MONITOR_DATASTORE_S3FS ] )
    then
        /bin/rm ${HOME}/runtime/PERFORMING_S3FS_CHECK ${HOME}/runtime/MONITOR_DATASTORE_S3FS
        exit
    else
        if ( [ -f ${HOME}/runtime/PERFORMING_S3FS_DATASTORE_CHECK ] && [ -f ${HOME}/runtime/MONITOR_DATASTORE_S3FS ] && [ "${count}" = "4" ] )
        then
            /bin/rm ${HOME}/runtime/PERFORMING_S3FS_DATASTORE_CHECK ${HOME}/runtime/MONITOR_DATASTORE_S3FS
